...
def main():
    ...
    ap.add_argument("-k", "--key", type=str, help="Seed for password generation")
    ap.add_argument("-l", "--length", default=15, type=int, help="Password length")
    ap.add_argument(
        "-s",
        "--salt",
        type=str,
    ...
    print(
        BANNER.format(
            ...
            color(
                key_seed + " (base on time.time())"
                if (args["key"] is None)
                else key_seed,
                OUTPUT_KEY_COLOR,
            ),
            color(
                get_file_full_path(args["salt"]) if args["salt"] else "None",
                OUTPUT_SALT_COLOR,
            ),
        )
    )
