...
ap.add_argument('-g', '--generate', default=False,
                action='store_true', help='生成 cli 直接调用式')
ap.add_argument('-c', '--cmd', default=False,
                action='store_true', help='cli 调用模式')
ap.add_argument('-i', '--interfaces', default=False,
                action='store_true', help='用户交互方式运行程序')
...
if isDefault:
    fgx(text='选择使用默认数据集')
    main(DEFAULT[0], DEFAULT[1], DEFAULT[2], DEFAULT[3],
         DEFAULT[4], DEFAULT[5], DEFAULT[6], DEFAULT[7])
elif isGenerate:
    fgx(text='选择生成 cmd 调用式')
    printCmd(DEFAULT[0], DEFAULT[1], DEFAULT[2], DEFAULT[3],
             DEFAULT[4], DEFAULT[5], DEFAULT[6], DEFAULT[7])
elif isCmd:
