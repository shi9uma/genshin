1. 缺什么 package 补什么
2. `./02-password-generator.sh --length 32 --seed your_key`，这个指令一开始的预期是根据日期、平台等指定 key 来生成固定密码
3. `python 01-password-generator.py --length 32 --seed your_key --salt salt.txt`
...
4. `03-ez-encrypt.py`
    ...
    6. tmp
        1. `03-ez-encrypt -i file --key ~/.03-ez-encrypt_key --salt ~/.03-ez-encrypt_salt enc`
        2. `03-ez-encrypt -d file.enc --key ~/.03-ez-encrypt_key --salt ~/.03-ez-encrypt_salt dec`